...
class CatBreedListVC: UIViewController
{
    ...
    override func viewDidLoad() {
        ...
        navigationItem.title = "Cat Breed List"
...
extension CatBreedListVC:UITableViewDelegate, UITableViewDataSource
{
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int
    {
        let countValue = filtered ? filteredBreeds.count : catBreeds.count
        if countValue == 0 { tableView.createTableEmptyLabel("No data") } else { tableView.backgroundView = UIView() }
        return countValue
    }
...
extension CatBreedListVC: UISearchControllerDelegate {
...
extension CatBreedListVC: UISearchBarDelegate {
...
extension CatBreedListVC: CatBreedDetailVCDelegate {
    func favChanged() {
        self.tableView.reloadData()
    }
}
